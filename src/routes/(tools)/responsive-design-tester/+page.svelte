<script>
    ...
    function closeSidebar() {
        clicked.mobile = false;
        clicked.desktop = false;
        clicked.tablet = false;
    }
...
<div class="container mx-auto max-w-screen-xl flex flex-col lg:flex-row overflow-hidden rounded-lg">
    <div class="side bg-blue-300 w-40 shadow-xl min-h-screen top-0 left-0 bottom-0">
        <button class="flex flex-col justify-between items-center" on:click={() => toggleSidebar("mobile")}>
            <FontAwesomeIcon class="w-14 h-14 py-6 px-14" icon={faMobile} />
        </button>
    
        <button class="flex flex-col justify-between items-center" on:click={() => toggleSidebar("desktop")}>
            <FontAwesomeIcon class="w-14 h-14 py-6 px-14" icon={faDesktop} />
        </button>
    
        <button class="flex flex-col justify-between items-center" on:click={() => toggleSidebar("tablet")}>
            <FontAwesomeIcon class="w-14 h-14 py-6 px-14" icon={faTabletScreenButton} />
        </button>
    </div>
    ...
    <main class="flex-1 py-16">
        ...
        <div class="content mt-10 overflow-hidden max-h-96">
            <object type="text/html" data={storedUrl} width={defaultWidth} height={defaultHeight} title="drop-down-page" style="overflow:auto;border:5px ridge blue">
            </object>
        </div>
                    ...
                    <ul>
                        {#each mobile as device}
                            {#each Object.entries(device) as [key, value]}
                                <button class="block w-full mb-4 text-left font-bold" on:click={() => (selectedDevice = key)}>
                                    {key}<br />
                                    <p class="text-sm text-gray-500">{value.width}x{value.height}</p>
                                </button>
                            {/each}
                        {/each}
                    </ul>
                ...
                <div class="submenu">
                    <ul>
                        {#each tablet as device}
                            {#each Object.entries(device) as [key, value]}
                                <button class="block w-full h-32 text-left font-bold" on:click={() => (selectedDevice = key)}>
                                    {key}<br />
                                    <p class="text-sm text-gray-500">{value.width}x{value.height}</p>
                                </button>
                            {/each}
...
<style>
    * {
    ...
    .submenu ul {
        ...
        transform: translate(-32px, -332px);
